...
const MyOrders = () => {
    ...
    const { data: orders, isLoading, refetch } = useQuery(['orders', user], () => fetch(`https://jantrik-server-side.up.railway.app/orders?email=${user?.email}`, {
        method: 'GET',
        headers: {
    ...
    const handleCancel = (id) => {
        const url = `https://jantrik-server-side.up.railway.app/orders/${id}`;
        fetch(url, {
            method: 'DELETE',
            headers: {
