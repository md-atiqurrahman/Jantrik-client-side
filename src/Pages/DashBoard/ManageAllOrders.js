...
const ManageAllOrders = () => {
    ...
    const { data: allOrders, isLoading, refetch } = useQuery(['allOders', user], () => fetch('https://jantrik-client-side.vercel.app/allOrders', {
        method: 'GET',
        headers: {
    ...
    const changeDeliveryStatus = (id) => {
        fetch(`https://jantrik-client-side.vercel.app/allOrders/${id}`, {
            method: 'PATCH',
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
